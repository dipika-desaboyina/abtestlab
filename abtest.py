...
if df.empty:
    st.info("Start by uploading a CSV file or switch to the autogenerated sample dataset in the sidebar. Here is a sample Kaggle dataset you can use for testing - https://www.kaggle.com/datasets/faviovaz/marketing-ab-testing.")
    st.stop()
...
st.write(
    f"Comparing `{test_variant}` (Variant B) against `{base_variant}` (Variant A) using {len(subset)} observations."
)
